function [ P_current_string ] = active_power_func( windspeed, irradiance )
% Compute active power for each string at several time instants %
...
for i = 2:length(setpoint_values)
    [~,idx_end]=min(abs(t-setpoint_times(i)));
...
[P_a_string(:,1:Ntb),~] = compute_pq_wtg(v_profile);
...
[P_a_pv,~] = compute_pq_pvg(solar_profile,Npv);
