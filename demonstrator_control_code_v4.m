...
sp_2_r = 100;                              % second setpoint 50 MVar
t_sp2_r =  1800;                           % second setpoint at 100 seconds
sp_3_r = -50;                              % third setpoint 0 MVar
t_sp3_r = 2700;                            % third setpoint at 200 seconds
sp_4_r = -50;                              % fourth setpoint 0 MVar
t_sp4_r = 2900;                            % fourth setpoint at 250 seconds
...
for j = 1:length(t)-1
    ...
    [Q_sp_strings, Q_available_wind, Q_available_solar, Q_total_available, error, u, Q_big, Opti_new, Opti_switch, count] = RPC_time_setpoint_v2( j, Ntb, Npv, Opti_new, Opti_switch, threshold_opt, Optimization_setpoint, Q_available_m, Q_sp_strings, Q_sp_pcc, Q_pcc, error, u, Q_big, count, distribution, z, optQs, Q_options, k_p, bs,t, T_i,Q_available_wind, Q_available_solar, Q_total_available, time_done_Q);

     % Implement rise time by adjusting reactive power set points %
    [Q_sp_strings, time_done_Q, rise_end_Q] = RPC_rise_time_test( Q_sp_strings,Q_sp_pcc,j, Ntb, Npv, N_rise_Q, rise_Q, rise_end_Q, time_done_Q, Q_available_m);
    ...
    if j > 2 && ( (rem(j,220)==0) || j==length(t)-1 )
