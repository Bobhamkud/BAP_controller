function [ P_a_tot, P_a_tot_pv, P_a_wind, P_sp_string, case_2] = APC_time_step_setpoint_v2(P_a_string, P_a_pv, P_sp_pcc,P_pcc,P_current_string,j, time_done, P_sp_string, a_p, Ntb, Npv,P_a_tot, P_a_tot_pv, P_a_wind, case_2)
...
%   active power set points for each string
    ...
    Delta_pcc = P_sp_pcc(j)-P_pcc(j);
     ...
     if j>1 && P_sp_pcc(j)~=P_sp_pcc(j-1)
        P_a_string_old = P_a_string(j,:); 
        P_a_tot_old = P_a_tot(j);
        P_a_string(j,:) = P_a_string(j,:) + 1000;
        P_a_tot(j) = sum(P_a_string(j,:));
     end
...
if j > 1
    ...
    if j>1 && P_sp_pcc(j)~=P_sp_pcc(j-1)
        P_a_tot(j) = P_a_tot_old; 
        P_a_string(j,:) = P_a_string_old;
    end
